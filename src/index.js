...
(async () => {
	...
	client.on('interactionCreate', async (interaction) => {
		...
		if (interaction.isCommand()) {
			...
			if ((interaction.customId === 'dm' || interaction.customId === 'comp' ||
					interaction.customId === 'casu' || interaction.customId === 'wingman'
				)) {
				matchType = {
					type: {
				...
				const boostCount = parseInt(interaction.customId.replace('boost-count-',
					''));
...
function calculateResult(experience, matchType, boostCount) {
	...
	const roundsNeeded = Math.ceil((xpNeeded - experience) / (factor * boostCount));
	return roundsNeeded;
}